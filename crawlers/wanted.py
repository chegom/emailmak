...
class WantedCrawler(BaseCrawler):
    ...
    async def fetch_json(self, url: str) -> Optional[Dict[str, Any]]:
        """JSON 데이터 가져오기"""
        try:
            headers = {'Referer': 'https://www.wanted.co.kr/'}
            if self.client:
                response = await self.client.get(url, headers=headers)
            else:
                client_headers = {**self.DEFAULT_HEADERS, **headers}
                async with httpx.AsyncClient(timeout=self.timeout, headers=client_headers) as client:
