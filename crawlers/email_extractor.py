...
class EmailExtractor:
    ...
    async def extract_from_url(self, url: str) -> List[str]:
        ...
        try:
            emails = await self._extract_with_client(url, verify=True)
        except httpx.TransportError as e:
            if not self._is_tls_verification_error(e):
                print(f"[WARN] Failed to extract from {url}: {e}")
                return []

            print(f"[WARN] TLS verification failed for {url}; retrying without certificate verification")
            emails = await self._extract_with_client(url, verify=False)

        return list(emails)

    async def _extract_with_client(self, url: str, verify: bool) -> Set[str]:
        emails: Set[str] = set()
    ...
    async def _extract_from_page(self, client: httpx.AsyncClient, url: str) -> Set[str]:
        ...
        try:
            ...
            if self._is_tls_verification_error(e):
                raise
            print(f"[WARN] Failed to extract from {url}: {e}")
            return set()
    ...
    def _is_tls_verification_error(self, exc: BaseException) -> bool:
